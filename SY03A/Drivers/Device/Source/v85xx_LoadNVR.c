...
/*------------------------------ P parameters --------------------------------*/
  ...
  pclk_mul = RTCTempData->APBClock / 6553600;
  ...
  if (checksum == (*NVR_RTC1_PCHECHSUM)) //checksum1 true
  {
    ...
    RTCTempData->RTCTempP2 = (int32_t)(rtc_data2 + (TempDelta<<8));
    ...
    RTC->ACP2 = (uint32_t)(rtc_data2 + (TempDelta<<8));
    RTC->ACP5 = (uint16_t)((rtc_data3 >> 16) & 0xFFFF);
    RTC->ACP6 = (uint16_t)((uint32_t)(rtc_data4 * pclk_mul)& 0xFFFF);
  ...
  else
  {
    ...
    else
    {
      ...
      RTCTempData->RTCTempP2 = -19746971 + (TempDelta << 8);
      RTCTempData->RTCTempP5 = 6444;
      RTCTempData->RTCTempP6 = (uint32_t)(1342 * pclk_mul);
      RTCTempData->RTCTempP7 = 0;

      /* Load data to ACPx register */
      RTC->ACP0 = (uint16_t)(-214);
      RTC->ACP1 = (uint16_t)(1060);
      RTC->ACP2 = (uint32_t)(-19746971 + (TempDelta << 8));
      RTC->ACP5 = (uint16_t)(6444);
      RTC->ACP6 = (uint16_t)(1342 * pclk_mul);
      RTC->ACP7 = (uint16_t)(0); 
...
/*--------------------------------- ACF200 -----------------------------------*/
  RTCTempData->RTCACF200 = (uint32_t)(pclk_mul * 0x320000);
  RTC->ACF200 = (uint32_t)(pclk_mul * 0x320000);
